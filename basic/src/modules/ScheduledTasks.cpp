...
/*
 * SaveScheduledTasks — 采集计划任务并字段级存入 SQLite3
...
char* SaveScheduledTasks(const char* paramsJson)
{
    ...
    cJSON_AddStringToObject(result, "module", "saveScheduledTasks");
    ...
    char* jsonStr = GetScheduledTasks(paramsJson);
    if (!jsonStr)
    {
        cJSON_AddStringToObject(result, "status", "error");
        cJSON_AddStringToObject(result, "message", "GetScheduledTasks failed");
    ...
    long long snapId = db.SaveScheduledTasks(jsonStr);
    FreeJsonString(jsonStr);
